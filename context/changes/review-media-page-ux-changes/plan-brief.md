...
The review page will become a full-window workspace for making informed import decisions. It adds useful safe metadata, quick multi-tag selection, and a focused comparison dialog for every similar imported picture, including a safe Substitute outcome that keeps the user's source originals untouched.
...
Every review candidate has a responsive information workspace, removable tags, recent-tag shortcuts, and best-effort metadata that clearly labels unavailable values. A reviewer can compare against every similar import, keep both, skip the current item, or substitute a selected old managed copy; replacement preserves a recoverable path on failure and transfers the normalized union of old/current tags.
...
| Metadata scope | Safe best-effort allowlist | Shows practical file/image/EXIF information without raw metadata or blocking review. |
...
- Full-window review layout, safe metadata, tag chips, and five recent imported tag shortcuts.
...
- Mutating source originals, raw EXIF/GPS, exact-match comparison, thumbnail generation, or a standalone duplicate-management view.

## Architecture / Approach

The Dioxus review component gains explicit metadata/tag/dialog state while native review DTOs add metadata and stable similar-match IDs. A recent-tags query uses the encrypted catalogue. Substitute is a dedicated native operation: publish the incoming managed copy, protect the old copy with a recovery link, update catalogue state transactionally, remove the old managed copy, then commit or compensate on failure.

## Phases at a Glance

| Phase | What it delivers | Key risk |
| --- | --- | --- |
| 1. Full-Window Review Information and Tags | Full-window metadata workspace and multi-tag review input | Unsupported metadata must never block decisions. |
...
- Some formats, especially unsupported images/videos, will have partial metadata; the UI must label it rather than fail review.

## Success Criteria (Summary)

- Review is fully usable across the window, including safe metadata and rapid tag selection.
